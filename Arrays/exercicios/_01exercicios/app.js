...
const newArr = [ ...randomNumbers ].filter(item => item %2 == 0)
console.log(newArr);
console.log("(1)--------------------------------------------------");
...
[ ...crazyNumbers ].filter(item => {
  if (item >= 500) {
    less500.push(item)
  }
})

console.log(`O array crazyNumbers possue ${less500.length} numeros abaixo de 500`);
console.log("(2)--------------------------------------------------");
...
const multNumbers = [ ...numbers ].map(item => {
...
[ ...tarantinoMovies ].sort((a, b) => a.release - b.release )
.filter(({ name, release }) => {

  if (release < 2000) {

    console.log(`
    filmes do Tarantino lançados antes do ano 2000
     Titulo ${name} Ano ${release}
    `);
